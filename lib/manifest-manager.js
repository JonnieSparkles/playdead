...
export class ManifestManager {
  ...
  async loadManualOverrides() {
    ...
    try {
      if (await fileExists(overridesPath)) {
        ...
        logger.info(`📋 Loaded ${Object.keys(overrides.paths || {}).length} manual TXID override(s)`);
        return overrides;
      }
    } catch (error) {
  ...
  async discoverAppFiles() {
    ...
    for (const file of allFiles) {
      // Skip manifest, tracker, and config files (but include manifest-overrides.json for change detection)
      if (file.name === 'manifest.json' || 
          file.name === 'deployment-tracker.json' ||
          file.name === 'upload-tags.json') {
        continue;
      }
