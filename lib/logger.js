...
class Logger {
  ...
  showDeploymentSummary(stats) {
    ...
    const uploadedSize = parseInt(stats.totalSize) || 0;
    const totalProjectSize = parseInt(stats.totalProjectSize) || uploadedSize;
    
    // Use actual total project size for accurate savings calculation
    const savedSize = Math.max(0, totalProjectSize - uploadedSize);
    const savingsPercentage = totalProjectSize > 0 ? ((savedSize / totalProjectSize) * 100).toFixed(2) : 0;

    table.push(
      ['Files Changed', changedFiles],
      ['Files Unchanged', unchangedFiles],
      ['Uploaded Size', this.formatBytes(uploadedSize)],
      ['Total Project Size', this.formatBytes(totalProjectSize)],
  ...
  showBalance(balance) {
    ...
    // Clear any existing progress line first to prevent text overlap
    if (this.isTTY) {
