...
class Logger {
  ...
  showDeploymentSummary(stats) {
    ...
    // Calculate savings
    const changedFiles = stats.changedFiles || 0;
    const unchangedFiles = stats.unchangedFiles || 0;
    const totalFiles = changedFiles + unchangedFiles;
    const uploadedSize = stats.totalSize || 0;
    
    // Estimate total size if we uploaded everything (rough calculation)
    // This is an approximation - in reality we'd need to calculate the actual total size
    const estimatedTotalSize = totalFiles > 0 ? Math.round((uploadedSize / changedFiles) * totalFiles) : uploadedSize;
    const savedSize = estimatedTotalSize - uploadedSize;
    const savingsPercentage = estimatedTotalSize > 0 ? Math.round((savedSize / estimatedTotalSize) * 100) : 0;

    table.push(
      ['Files Changed', changedFiles],
      ['Files Unchanged', unchangedFiles],
      ['Uploaded Size', this.formatBytes(uploadedSize)],
      ['Estimated Full Size', this.formatBytes(estimatedTotalSize)],
      ['💰 Savings', `${this.formatBytes(savedSize)} (${savingsPercentage}%)`],
  ...
  // Helper method to format bytes (moved from utils for use in logger)
  formatBytes(bytes) {
