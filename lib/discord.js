...
function extractGatewayHost(gatewayUrl) {
  // Extract host from full gateway URL (e.g., https://arweave.net -> arweave.net)
  try {
    const url = new URL(gatewayUrl);
    return url.host;
  } catch (error) {
    // Fallback: remove protocol if URL parsing fails
    return gatewayUrl.replace(/^https?:\/\//, '');
  }
}
...
async function formatDeploymentMessage(deploymentData) {
  ...
  try {
    ...
    const gatewayHost = extractGatewayHost(config?.arweaveGateway || 'https://arweave.net');
    const deploymentUrl = linkMatch ? linkMatch[0] : 
      (undername.includes('_') ? 
        `https://${undername}.${gatewayHost}` : 
        `https://${undername}_${rootArnsName}.${gatewayHost}`);
    ...
    const deploymentUrl = `https://${undername}_${rootArnsName}.${gatewayHost}`;
