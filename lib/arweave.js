...
// ---------- upload functions ----------
export async function uploadToArweave(mediaBuffer, contentType = 'application/octet-stream', appName = null, jwk = null, customTags = [], appPath = null) {
  try {
    ...
    // Load config for shared credits
    const config = loadConfig();
    ...
    const tagManager = new TagManager(appPath);
    await tagManager.load();
    const configTags = tagManager.getAllTags();
...
export async function uploadManifest(pathMap, indexPath = 'index.html', appName = null, jwk = null, appPath = null) {
  try {
